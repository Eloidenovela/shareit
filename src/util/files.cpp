...
namespace files 
{
    std::optional<std::vector<char>> read(const std::string & file_name) {
        ...
        auto get_absolute_path = util::get_absolute_path(file_name);
        
        if (util::is_binary(file_name)) {
            auto ifile = std::ifstream(get_absolute_path, std::ios::in | std::ios::binary);

            if (ifile.is_open()) {
                try {
                    auto file_size = std::filesystem::file_size(get_absolute_path);
            ...
            auto ifile = std::ifstream(get_absolute_path, std::ios::in);
